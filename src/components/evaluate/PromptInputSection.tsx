...
interface PromptInputSectionProps {
  onAnalysisResult?: (result: any, error?: string) => void;
...
const PromptInputSection: React.FC<PromptInputSectionProps> = ({
  ...
  const handleAnalyzePrompt = async () => {
    if (promptInput.length < 10 || isAnalyzing) return;
    setIsAnalyzing(true);
    try {
      ...
      if (!response.ok) {
        ...
        onAnalysisResult?.(null, errorText || 'Failed to analyze prompt');
        return;
      } 
      ...
      onAnalysisResult?.(null, error?.message || 'Error analyzing prompt');
    } finally {
      onLoadingChange?.(false);
      setIsAnalyzing(false);
    }
  ...
  const buttonText = isAnalyzing
    ...
    : promptInput.length >= 10
      ? '> Analyze This Prompt'
      : '> Get Prompt Vitals';
  const isButtonDisabled = promptInput.length < 10 || isAnalyzing;
